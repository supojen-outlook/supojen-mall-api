...
/// 
/// 設計模式：
/// - 實作 IRequest<int>，表示這是一個查詢請求，回傳整數（產品總數）
/// - 遵循 CQRS (Command Query Responsibility Segregation) 原則
/// - 與 ProductCountQueryHandler 配合使用，完成查詢
/// 
/// 使用場景：
/// - 儀表板顯示產品總數
/// - 統計報表生成
...
/// 
/// 職責：
/// - 接收 ProductCountQuery 請求
/// - 從資料庫取得產品的總數量
/// - 優先使用估計數量，提升效能
/// 
/// 設計模式：
/// - 實作 IRequestHandler<ProductCountQuery, int> 介面
/// - 遵循單一職責原則 (SRP)
/// - 使用依賴注入 (DI) 取得所需服務
/// 
/// 生命週期：
...
public class ProductCountQueryHandler : IRequestHandler<ProductCountQuery, int>
{
    ...
    /// 
    /// 注意事項：
    /// - ProductRepository 目前沒有定義額外方法
    /// - 完全依賴父類別 Repository<Product> 提供的功能
    ...
    /// 建構函式 - 初始化處理器並注入依賴服務
    /// </summary>
    /// <param name="repository">產品倉儲，用於查詢產品數量</param>
    public ProductCountQueryHandler(IProductRepository repository)
    {
    ...
    /// - 都在估計數量不可用時執行精確計數
    /// </summary>
    /// <param name="request">產品數量查詢請求物件（不包含任何屬性）</param>
    /// <returns>產品總數（整數）</returns>
    public async Task<int> HandleAsync(ProductCountQuery request)
    {
        // ========== 第一步：嘗試取得估計數量 ==========
