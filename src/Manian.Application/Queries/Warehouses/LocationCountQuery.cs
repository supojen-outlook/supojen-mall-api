...
/// 
/// 設計模式：
/// - 實作 IRequest<int>，表示這是一個查詢請求，回傳整數（儲位總數）
/// - 遵循 CQRS (Command Query Responsibility Segregation) 原則
/// - 與 LocationCountQueryHandler 配合使用，完成查詢
/// 
/// 使用場景：
/// - 儀表板顯示儲位總數
/// - 統計報表生成
...
/// </summary>
public class LocationCountQuery : IRequest<int>;

/// <summary>
/// 儲位數量查詢處理器
/// 
/// 職責：
/// - 接收 LocationCountQuery 請求
/// - 從資料庫取得儲位的總數量
/// - 優先使用估計數量，提升效能
/// 
/// 設計模式：
/// - 實作 IRequestHandler<LocationCountQuery, int> 介面
/// - 遵循單一職責原則 (SRP)
/// - 使用依賴注入 (DI) 取得所需服務
/// 
/// 生命週期：
...
/// </summary>
public class LocationCountQueryHandler : IRequestHandler<LocationCountQuery, int>
{
    /// <summary>
    /// 儲位倉儲介面
    /// 
    ...
    /// 
    /// 注意事項：
    /// - LocationRepository 目前沒有定義額外方法
    /// - 完全依賴父類別 Repository<Location> 提供的功能
    ...
    /// 建構函式 - 初始化處理器並注入依賴服務
    /// </summary>
    /// <param name="repository">儲位倉儲，用於查詢儲位數量</param>
    public LocationCountQueryHandler(ILocationRepository repository)
    {
    ...
    /// - 都在估計數量不可用時執行精確計數
    /// </summary>
    /// <param name="request">儲位數量查詢請求物件（不包含任何屬性）</param>
    /// <returns>儲位總數（整數）</returns>
    public async Task<int> HandleAsync(LocationCountQuery request)
    {
        // ========== 第一步：嘗試取得估計數量 ==========
