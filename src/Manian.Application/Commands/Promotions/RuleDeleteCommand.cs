...
/// - 軟刪除可能比硬刪除更安全
/// </summary>
public class RuleDeleteCommand : IRequest
{
    /// <summary>
    /// 規則唯一識別碼
    /// 
...
/// <summary>
/// 刪除促銷規則命令處理器 (CQRS 模式中的 Command Handler)
/// 
/// 職責：
/// - 接收 RuleDeleteCommand 命令
/// - 查詢規則是否存在
/// - 執行刪除操作
/// 
/// 設計模式：
/// - 實作 IRequestHandler<RuleDeleteCommand> 介面
/// - 遵循單一職責原則 (SRP)
/// - 使用依賴注入 (DI) 取得所需服務
/// 
/// 生命週期：
...
/// </summary>
internal class RuleDeleteHandler : IRequestHandler<RuleDeleteCommand>
{
    /// <summary>
    /// 促銷活動倉儲介面
    /// 
    ...
    /// - 擴展了 GetRuleAsync、DeleteRule 等方法
    /// 
    /// 介面定義：
    ...
    /// 建構函式 - 初始化處理器並注入依賴服務
    /// </summary>
    /// <param name="repository">促銷活動倉儲，用於查詢和刪除規則</param>
    public RuleDeleteHandler(IPromotionRepository repository)
    {
    ...
    /// </summary>
    /// <param name="request">刪除促銷規則命令物件，包含規則 ID</param>
    /// <returns>一個表示非同步操作的工作 (Task)</returns>
    public async Task HandleAsync(RuleDeleteCommand request)
    {
        // ========== 第一步：根據 ID 查詢規則實體 ==========
        ...
        // ========== 第三步：刪除規則 ==========
        // 使用 IPromotionRepository.DeleteRule() 刪除規則
        // 注意：這會從資料庫中永久刪除該筆記錄（硬刪除）
        // 如果需要軟刪除，應該改為更新規則的狀態欄位
        // 這個方法只會標記實體為待刪除，不會立即寫入資料庫
        // 根據資料庫約束，如果規則有關聯的訂單，刪除會失敗
