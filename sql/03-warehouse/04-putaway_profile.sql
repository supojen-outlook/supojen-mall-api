...
-- 假設 skus 表有 SKU：
-- 藍色茶具組 (id=1001001), 白色茶具組 (id=1001002), 銀色 Audi (id=3001001)
*/

INSERT INTO putaway_profiles (id, sku_id, preferred_bin_id) VALUES
    (1, 1001001, 101),  -- 藍色茶具組首選 A01-01
    (2, 1001002, 102),  -- 白色茶具組首選 A01-02
    (3, 3001001, 201);  -- 銀色 Audi 首選 B01-01
...
SELECT 
    ...
    s.name AS sku_name,
    p.preferred_bin_id,
    l.bin_code AS preferred_bin
FROM putaway_profiles p
JOIN skus s ON p.sku_id = s.id
LEFT JOIN locations l ON p.preferred_bin_id = l.id
WHERE s.id = 1001001;

-- 2. 查詢某個儲位被哪些 SKU 設為首選
SELECT 
    l.bin_code,
    s.sku_code,
    s.name
FROM putaway_profiles p
JOIN skus s ON p.sku_id = s.id
JOIN locations l ON p.preferred_bin_id = l.id
WHERE l.id = 101;
