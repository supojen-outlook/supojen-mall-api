...
/*
-- 對應 inventories 範例：
-- 藍色茶具組 (sku_id=1001001) 在 A01-01 (location_id=101) 有 50 個
*/

INSERT INTO inventory_transactions (id, sku_id, transaction_type, quantity, reference_type, reference_id, note) VALUES
    -- 初始入庫
    (1, 1001001, 'in', 50, 'purchase', 2001, '採購入庫：50個藍色茶具組'),
    
    -- 訂單出貨
    (2, 1001001, 'out', -10, 'order', 3001, '訂單出貨：訂單 #3001 出貨 10個'),
    (3, 1001001, 'out', -5, 'order', 3002, '訂單出貨：訂單 #3002 出貨 5個'),
    
    -- 庫存調整（報廢、損壞）
    (4, 1001001, 'adjust', -2, 'adjust', 4001, '報廢：2個外盒損壞'),
    
    -- 退貨入庫
    (5, 1001001, 'in', 3, 'order', 3001, '退貨入庫：訂單 #3001 退貨 3個'),
    
    -- 盤點調整
    (6, 1001001, 'adjust', 1, 'adjust', 4002, '盤點調整：盤盈 1個');
...
-- =============================================================================
-- 觸發器 (Triggers)
-- 用途：可選，用於自動記錄交易（如果應用層會寫入就不需要）
-- =============================================================================

/*
-- 如果希望在更新 inventories 時自動寫入交易紀錄，可以用這個觸發器
-- 但建議由應用層手動寫入，比較好控制業務邏輯

CREATE OR REPLACE FUNCTION fn_auto_log_inventory_transaction()
RETURNS TRIGGER AS $$
BEGIN
    IF OLD.quantity_on_hand != NEW.quantity_on_hand THEN
        INSERT INTO inventory_transactions (
            id, sku_id, transaction_type, quantity, 
            reference_type, reference_id, note
        ) VALUES (
            nextval('inventory_transactions_id_seq'), 
            NEW.sku_id,
            CASE 
                WHEN NEW.quantity_on_hand > OLD.quantity_on_hand THEN 'in'
                ELSE 'out'
            END,
...
CREATE TRIGGER trg_inventories_after_update
    AFTER UPDATE OF quantity_on_hand ON inventories
    FOR EACH ROW
    WHEN (OLD.quantity_on_hand IS DISTINCT FROM NEW.quantity_on_hand)
    EXECUTE FUNCTION fn_auto_log_inventory_transaction();
*/
