...
-- =============================================================================
-- 範例資料 (Sample Data)
-- 用途：對應 promotions 和 orders 範例，記錄使用情況
-- =============================================================================

/*
-- 假設 orders 有訂單：
-- 訂單 1: ORD-001 (user_id=1001, total=1200)
-- 訂單 2: ORD-002 (user_id=1001, total=800)
*/

INSERT INTO promotion_usages (id, promotion_id, user_id, order_id, used_quantity, rule_snapshot) VALUES
    (1, 2, 1001, 1, 1, '{"rule_type":"full_reduction","threshold_amount":1000,"discount_amount":100}'::JSONB),
    (2, 2, 1001, 2, 1, '{"rule_type":"full_reduction","threshold_amount":1000,"discount_amount":100}'::JSONB);
...
SELECT 
    p.name,
    p.limit_total,
    COUNT(pu.id) AS actual_used
FROM promotions p
LEFT JOIN promotion_usages pu ON p.id = pu.promotion_id
WHERE p.id = 2
GROUP BY p.id, p.name, p.limit_total;
